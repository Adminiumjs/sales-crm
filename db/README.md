...
`docker compose up` gives you a working product: `crm-db` creates the
...
By default these talk to the `crm-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/meridian npm run demo:status
```
